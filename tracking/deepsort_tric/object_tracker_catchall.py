            ...
            if return_value:
                ...
                if total_frames % skip_frames == 0:
                    ...
                    for i in range(num_objects):
                        ...
                        if class_name not in allowed_classes:
                            ...
                            if class_name == 'LicensePlate':
                                # crop license plate region from the image
                                plate_img = frame[ymin:ymax, xmin:xmax]
                                # pass the cropped image to your license plate recognition script
                                plate_number = self.recognize_license_plate(plate_img)
                            elif class_name == 'Tricycle': 
                                tric_img = frame[ymin:ymax, xmin:xmax]
                                tric_color = self.recognize_color(tric_img)'''
                ...
                # Delete memory of old tracks.
                    ...
                    if len(memory) > 50:
                        ...
                        # Draw total count.
                    cv2.putText(frame, "Total Vehicle: {} ({} right, {} left)".format(str(total_counter), str(up_count),
                                str(down_count)), (int(0.05 * frame.shape[1]), int(0.1 * frame.shape[0])), 0,
                                1.3e-3 * frame.shape[0], (0, 255, 255), 2)
                    if show_detections:
                        track_dict = {}
