...
class VehiclesCounting():
    ...
    '''def _plate_within_roi(self, bbox, roi):
        ...
        # Check if all four corners of the bounding box are within the ROI
        if roi_xmin <= xmin <= roi_xmax and roi_ymin <= ymin <= roi_ymax \
                and roi_xmin <= xmax <= roi_xmax and roi_ymin <= ymax <= roi_ymax:
            return True
        else:
            return False'''
        ...
        while True:
            ...
            if return_value:
                ...
                if total_frames % skip_frames == 0:
                    ...
                    rectangle_bottom_right = (1600, 800)
                    ...
                    for track in tracker.tracks:
                        ...
                        if (rectangle_top_left[0] <= midpoint[0] <= rectangle_bottom_right[0] and
                            ...
                            # Object is inside the rectangle, count it
                            ...
                            # Draw a green rectangle around the counted object
                            cv2.rectangle(frame, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), color, 2)

                            # Draw count at the top of the bounding box
                            text = f"{class_name}: {class_counter[class_name]}"
                            cv2.putText(frame, text, (int(bbox[0]), int(bbox[1]) - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
                        ...
                        if self._intersect(midpoint, previous_midpoint, line[0], line[1]) and track.track_id not in already_counted:
                                ...
                                '''if self._plate_within_roi(bbox, roi_vertices):
                                    print("Displaying plate_num:", plate_num)
                                ...
                                cv2.rectangle(frame, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), (255, 255, 255), 2)  # WHITE BOX
                                        ...
                                        # Draw detected class text above the bounding box
