                ...
                if total_frames % skip_frames == 0:
                    ...
                    for track in tracker.tracks:
                        ...
                        if self._intersect(midpoint, previous_midpoint, line[0], line[1]) and track.track_id not in already_counted:
                                ...
                                if class_name in ['Tricycle_Jeep', 'Tricycle_Sikad']:
                                    xmin, ymin, xmax, ymax = map(int, bbox) 
                                    tric_img = frame[int(ymin):int(ymax), int(xmin):int(xmax)]
                                    tric_color = self.recognize_color(tric_img)
                                    cv2.putText(frame, str(tric_color), (int(bbox[0]), int(bbox[1])), 0,
