...
import tempfile
#from read_plates_cv import perform_yolo_detection, get_detected_classes
#from easypaddleocr.easy_paddle_ocr.algorithm.text_recognizer import TextRecognizer

# Initialize the text recognizer
#text_recognizer = TextRecognizer()

class VehiclesCounting():
    def __init__(self, file_counter_log_name, framework='tf', weights='./checkpoints/tricplate/',
        ...
        while True:
            ...
            if return_value:
                ...
                if total_frames % skip_frames == 0:
                    ...
                    x1 = 0
                    y1 = 1100#int(self._detect_line_position * frame.shape[0] + yp)
                    x2 = int(frame.shape[1])
                    y2 = 700#int(self._detect_line_position * frame.shape[0] + yp)
                    ...
                    for track in tracker.tracks:
                        ...
                        if self._intersect(midpoint, previous_midpoint, line[0], line[1]) and track.track_id not in already_counted:
                                ...
                                # Path to the folder where cropped license plate images are saved
                                cropped_images_folder = './media/detected_plates'
                                detected_frames_folder = './media/detected_frames'
                                #for cropping license_plates
                                if class_name in ['License_Plate']:
                                    xmin, ymin, xmax, ymax = map(int, bbox)
                                     # Add allowance to the ymin and ymax coordinates
                                    '''allowance_y = 10  # Adjust this value as needed
                                    allowance_x = 10
                                    ymin_with_allowance = max(0, ymin - allowance_y)
                                    ymax_with_allowance = min(frame.shape[0], ymax + allowance_y)
                                    xmin_with_allowance = max(0, xmin - allowance_x)
                                    xmax_with_allowance = min(frame.shape[1], xmax + allowance_x)'''
                                    
                                    # Crop the license plate image with allowance
                                    #plate_img = frame[ymin_with_allowance:ymax_with_allowance, xmin_with_allowance:xmax_with_allowance]
                                    ...
                                    frame_box = cv2.rectangle(frame.copy(), (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), (255,255,255), 2)
                                    ...
                                    if track_id not in saved_track_ids and self._intersect(midpoint, previous_midpoint, line[0], line[1]):
                                        ...
                                        image_name = "".join(prediction["detected_classes"]) + ".jpg"
                                        
                                        if not os.path.exists(cropped_images_folder):
                                            os.mkdir(cropped_images_folder)
                                        cropped_image_path = os.path.join(cropped_images_folder, image_name)
                                        cv2.imwrite(cropped_image_path, plate_resized)

                                        if not os.path.exists(detected_frames_folder):
                                            os.mkdir(detected_frames_folder)
                                        detected_frame_path = os.path.join(detected_frames_folder, image_name)
                                        cv2.imwrite(detected_frame_path, frame_box)
